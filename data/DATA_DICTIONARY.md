...
|---|---|---|
| company_id | Synthetic row ID | SYN0001, SYN0002, ... |
| profile | Readiness profile used (synthetic data only; not real data) | `Early_Stage_Adopter`, `Well_Funded`, `Tech_Focused`, `Traditional_Company`, `Developing`, `Leadership_Driven`, `Governance_Focused`, `Advanced_Adopter` |
...
| region | Country of operation | `UAE`, `UK`, `India`, `USA`, `Other` |
| current_ai_stage | Current stage of AI adoption | `Not considering`, `Exploring`, `Piloting`, `Actively Using`, `Fully Integrated` |

These columns are not scored. They are only used to provide additional information about the company.
...
**Reverse-worded items are marked below.** These questions describe a barrier, so agreeing with them means lower readiness. The tool adjusts these answers before calculating the score. The values in the CSV are kept as the respondent originally answered them.
